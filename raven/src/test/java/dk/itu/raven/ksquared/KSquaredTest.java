...
public class KSquaredTest {
    ...
    public void searchValuesInWindowTest() {
        ...
        List<IResult> filteredValues = k2Raster.searchValuesInWindow(buffer, height - 1 - buffer, buffer,
                width - 1 - buffer,
                JoinFilterFunctions.rangeFilter(filterLow, filterHigh), new Offset<>(0, 0));
        for (IResult range : filteredValues) {
            for (var res : range) {
                actual[res.x][res.y] = true;
            }
        }
