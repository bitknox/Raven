 ...
 * <p>
 * The bit set is backed by an integer array where each {@code int} value
 * represents 32 consecutive
 * bits packed together. Though the bit set is read-only, the provided integer
 * array is used as is.
 * Be sure you will not modify the array beyond this structure. The bit set can
 * handle up to
 * {@link Integer#MAX_VALUE} bits.
 * <p>
 * The rank implementation follows Jacobson's approach (G. Jacobson. Succinct
 * static data
 * structures).
 *
 * @author Yauheni Shahun
 */
public class IntRank implements Serializable {
    ...
    /**
     * Rank directory that stores absolute cumulative ranks of the large blocks of
     * 256 bits. The
     * length of the directory is the number of large blocks plus one. That last
     * rank value is
     * reserved to hold the total rank of the bit set.
     */
    protected final int[] largeRankDirectory;
    /**
     * Rank directory that stores ranks of the small blocks of 32 bits. The ranks
     * are calculated
     * cumulatively within the large block that the small blocks belong to. The
     * small blocks
     * correspond to the {@code int} elements of the bit array.
     * <p>
     * The maximum expected rank within the large block is 32*7=224 what fits the
     * {@code byte} data
     ...
     * @throws IllegalArgumentException if the size is less than or equal to 0, or
     *                                  the size is greater
     *                                  than the number of bits in the given array
     */
    public IntRank(int[] vector, int size) {
        if (size <= 0 || size > vector.length * BLOCK_SIZE) {
            throw new IllegalArgumentException("Invalid vector or size: " + size);
        }

        this.vector = vector;
        this.size = size;
        int largeBlockCount = (int) (((long) vector.length + SMALL_BLOCK_COUNT - 1) / SMALL_BLOCK_COUNT) + 1;
    ...
    public int rank(int index) {
        int rank = largeRankDirectory[index >> 8]; // same as division by LARGE_BLOCK_BIT_COUNT

        int blockIndex = index >> 5; // same as division by BLOCK_SIZE
        rank += smallRankDirectory[blockIndex] & 0xFF; // Upcast byte to int.

        int remainder = index & 31; // % BLOCK_SIZE
