...
public class RavenJoin extends AbstractRavenJoin {
    ...
    public RavenJoin(AbstractK2Raster k2Raster, RTree<Object, Geometry> tree,
    ...
    public RavenJoin(AbstractK2Raster k2Raster, RTree<Object, Geometry> tree, Size imageSize) {
    ...
    public Collection<IResult> findPixelRanges(java.awt.Rectangle rasterBounding, boolean[] inRanges,
            Map<Long, Integer> intersections, Square area, long minValue, long maxValue, int k2Index, boolean prob) {
        ...
        for (List<PixelRange> ranges : layers) {
            ...
            if (prob) {
                k2Raster.searchValuesInRanges(ranges, out, offset, r1, r2, c1, c2, rangeLimits, rangeCountPrefixsum,
                        function, k2Index - 1, 0, area.getTopX() - offset.getX(), area.getTopY() - offset.getY(),
     ...
     * @param rasterBounding teh bounding box of the sub-matrix corresponding to
     * the node with index {@code pk} in the k2 raster tree
     * @return A collection of pixels that are contained in the vector shape
     * described by {@code polygon}
     */
    protected Collection<IResult> extractCellsPolygon(Polygon polygon, int pk, Square area, long minValue,
        ...
        return findPixelRanges(rasterBounding, inRanges, intersections, area, minValue, maxValue, pk, prob);
    }

    // based loosely on:
    // https://bitbucket.org/bdlabucr/beast/src/master/raptor/src/main/java/edu/ucr/cs/bdlab/raptor/Intersections.java
    private void extractCells(Leaf<Object, Geometry> pr, int pk, Square area, long minValue,
            long maxValue, java.awt.Rectangle rasterBounding,
            JoinResult def, boolean prob) {
        for (Entry<Object, Geometry> entry : ((Leaf<Object, Geometry>) pr).entries()) {
     ...
     * @param rasterBounding the bounding box of the sum-matrix corresponding to
     * the node with index {@code pk} in the k2 raster tree
     * @param def the list all the pixelranges should be added to
     */
    private void addDescendantsLeaves(NonLeaf<Object, Geometry> pr, int pk, Square area, long minValue,
            long maxValue, java.awt.Rectangle rasterBounding,
            JoinResult def, boolean prob) {
        for (Node<Object, Geometry> n : pr.children()) {
            if (TreeExtensions.isLeaf(n)) {
                extractCells((Leaf<Object, Geometry>) n, pk, area, minValue, maxValue, rasterBounding, def, prob);
            } else {
                addDescendantsLeaves((NonLeaf<Object, Geometry>) n, pk, area, minValue, maxValue, rasterBounding, def,
                        prob);
            }
        }
    }
    ...
    /**
     * Finds the smallest node of the k2 raster tree that fully contains the
     * given bounding box
     *
     * @param k2Index the index of the starting node in the k2 raster tree. This
     * node should always contain {@code bounding}
     * @param rasterBounding the bounding box of the sum-matrix corresponding to
     * the node with index {@code k2Index} in the k2 raster tree
     ...
     * @param min the value of VMin for the node with index {@code k2Index} in
     * the k2 raster tree
     * @param max the value of VMax for the node with index {@code k2Index} in
     * the k2 raster tree
     ...
     * <li>min' is the value of VMin for the node with index k2Index' in the k2
     * raster tree</li>
     * <li>max' is the value of VMax for the node with index k2Index' in the k2
     * raster tree</li>
     * </ul>
     */
    ...
    /**
     * More precise than {@code checkQuadrant}, but slower checks if the MBR of
     * a k2 raster node intersects with a given bounding box
     *
     * @param k2Index the index of the k2 raster node
     * @param rasterBounding the bounding box of the sub-matrix corresponding to
     * the node with index {@code k2Index} in the k2 raster tree
    ...
    protected JoinResult joinImplementation(IRasterFilterFunction function) {
        ...
        Stack<Tuple5<Node<Object, Geometry>, Integer, Square, Long, Long>> S = new Stack<>();

        Pair<Long, Long> minMax = k2Raster.getValueRange();

        for (Node<Object, Geometry> node : TreeExtensions.getChildren(tree.root().get())) {
        ...
        while (!S.empty()) {
            Tuple5<Node<Object, Geometry>, Integer, Square, Long, Long> p = S.pop();
            ...
            switch (checked.a) {
                case TotalOverlap:
                    if (TreeExtensions.isLeaf(p.a)) {
                        ...
                        extractCells((Leaf<Object, Geometry>) p.a, checked.b, checked.c, checked.d, checked.e, rect,
                                def, false);
                    } else {
                        addDescendantsLeaves((NonLeaf<Object, Geometry>) p.a, checked.b, checked.c, checked.d,
                                checked.e, rect, def, false);
                    }
                    break;
                case PossibleOverlap:
                    if (!TreeExtensions.isLeaf(p.a)) {
                        for (Node<Object, Geometry> c : ((NonLeaf<Object, Geometry>) p.a).children()) {
                            S.push(new Tuple5<Node<Object, Geometry>, Integer, Square, Long, Long>(c, checked.b,
                                    checked.c,
                                    checked.d, checked.e));
                        }
                    } else {
                        ...
                        switch (overlap) {
                            case TotalOverlap:
                                extractCells((Leaf<Object, Geometry>) p.a, checked.b, checked.c, checked.d, checked.e,
                                        rect, def, false);
                                break;
                            case PartialOverlap:
                                extractCells((Leaf<Object, Geometry>) p.a, checked.b, checked.c, checked.d, checked.e,
