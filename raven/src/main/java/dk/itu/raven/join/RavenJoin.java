...
public class RavenJoin extends AbstractRavenJoin {
    ...
    public Collection<IResult> findPixelRanges(java.awt.Rectangle rasterBounding, boolean[] inRanges,
            ...
            if (y != oldY) { // new pixel-line
                ...
                for (int j = oldY + 1; j <= y; j++) {
                    if (inRange) {
                        ...
                        addRange(rasterBounding, ranges, r, x1, x2);
                    }
        ...
        if (prob) {
            for (PixelRange range : ranges) {
                k2Raster.searchValuesInWindow(range.row - offset.getY(), range.row - offset.getY(), range.x1 - offset.getX(),
                        range.x2 - offset.getX(), function, out, offset);
            ...
            if (k2Raster.getResultCreator().hasValues()) {
                if (minValue == maxValue) {
                    ...
                    for (PixelRange range : ranges) {
                        k2Raster.searchValuesInWindow(range.row - offset.getY(), range.row - offset.getY(), range.x1 - offset.getX(), range.x2 - offset.getX(), function, out, offset);
                    }
                }
            } else {
                for (PixelRange range : ranges) {
    ...
    private void addRange(java.awt.Rectangle rasterBounding, List<PixelRange> ranges, int r,
            int x1, int x2) {
