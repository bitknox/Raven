 ...
 * by the <code>GeoTiffIIOMetadataDecoder</code> for the purposes of
 ...
 * with the <code>
 * CoordinateSystemAuthorityFactory</code> specified by the
 * <CODE>GeoTiffFormat</CODE> instance
 * which created the reader. The image specific metadata should then be set with
 * the appropriate
 * accessor methods. Finally, the <code>createCoordinateSystem()</code> method
 * is called to produce
...
public final class GeoTiffMetadata2CRSAdapter {
     ...
     *
     * @param hints a map of hints to locate the authority and object factories.
     *              (can be null)
     */
    public GeoTiffMetadata2CRSAdapter(Hints hints) {
        ...
        // Might alleviate a rare deadlock
        // https://osgeo-org.atlassian.net/browse/GEOT-5405

        this.hints.put(Hints.DEFAULT_COORDINATE_REFERENCE_SYSTEM, DefaultCRS);
        this.hints.put(Hints.FORCE_LONGITUDE_FIRST_AXIS_ORDER, true);
    ...
    /**
     * This method creates a <code>CoordinateReferenceSystem</code> object from the
     * metadata which
     * has been set earlier. If it cannot create the
     * <code>CoordinateReferenceSystem</code>, then
     ...
     *         file data
     * @throws IOException                   if there is unexpected data in the
     *                                       GeoKey tags.
     * @throws NullPointerException          if the <code>csAuthorityFactory</code>,
     *                                       <code>datumFactory
     *     </code>                        , <code>crsFactory</code> or
     *                                       <code>metadata</code> are uninitialized
     * @throws UnsupportedOperationException if the coordinate system specified by
     *                                       the GeoTiff file
     *                                       is not supported.
     */
        ...
        switch (getGeoKeyAsInt(GTModelTypeGeoKey, metadata)) {
            ...
            default:
                throw new UnsupportedOperationException(String.format(
                        "GeoTiffMetadata2CRSAdapter::createCoordinateSystem:Only Geographic & Projected Systems are supported. %d is unsupported!",
                        getGeoKeyAsInt(GTModelTypeGeoKey, metadata)));
        }
    }

    /**
     * This code is responsible for creating a projected coordinate reference system
     * as specified in
     * the GeoTiff specification. User defined values are supported throughout the
     * evolution of this
     * specification with except of the coordinate transformation which must be one
     * of the supported
     * types.
     *
    ...
    private ProjectedCRS createProjectedCoordinateReferenceSystem(
        ...
        try {
            ...
            final ProjectedCRS pcrs = (ProjectedCRS) allAuthoritiesFactory.createCoordinateReferenceSystem(
                    projCode);
            // //
            //
            ...
            //
            // //
            boolean isDefaultUnit = metadata.getGeoKey(ProjLinearUnitsGeoKey) == null
                    && linearUnit != null
     ...
     * GeoTiff
     * specification. User defined values are supported for all the possible levels
     * of the above
    ...
    private GeographicCRS createGeographicCoordinateReferenceSystem(
        ...
        if (tempCode == null || tempCode.equals(GTUserDefinedGeoKey_String)) {
            ...
            try {
                ...
                gcs = (GeographicCRS) allAuthoritiesFactory.createCoordinateReferenceSystem(
                        geogCode.toString());
                if (angularUnit != null
    ...
    /**
     * Getting a specified geotiff geo key as a int. It is somehow tolerant in the
     * sense that in
     ...
     * respecting
     * ALWAYS the OGC {@link PixelInCell#CELL_CENTER} convention for the
     * {@link ImageDatum} of the
     * underlying {@link ImageCRS}.
     *
     * @see <a href=
     *      "http://lists.maptools.org/pipermail/geotiff/2006-January/000213.html">this
     *      email post</a>
     * @param metadata containing the information to build the {@link MathTransform}
     *                 for going from
     *                 grid to world.
     ...
     * @return an instance of the {@link PixelInCell} enum that capture the raster
     *         type.
     */
    ...
    /**
     * Getting a specified geotiff geo key as a double. It is somehow tolerant in
     * the sense that in
     ...
     *
     * @param linearUnit is the UoM that this {@link ProjectedCRS} will use. It
     *                   could be null.
    ...
    /**
     * Clean the provided parameters <code>tiffName</code> from strange strings like
     * it happens with
     * erdas imagine.
     *
    ...
    /**
     * Creates a {@link CartesianCS} for a {@link ProjectedCRS} given the provided
     * {@link Unit}.
     *
    ...
    /**
     * Creating a prime meridian for the gcs we are creating at an higher level. As
     * usual this
     * method tries to follow the geotiff specification.
     *
     * @param linearUnit to use for building this {@link PrimeMeridian}.
     * @return a {@link PrimeMeridian} built using the provided {@link Unit} and the
     *         provided
     *         metadata.
     */
            ...
            if (pmCode != null) {
                if (pmCode.equals(GTUserDefinedGeoKey_String)) {
                    try {
                        ...
                        final String pmValue = metadata.getGeoKey(GeogPrimeMeridianLongGeoKey);
                        ...
                        final IOException io = new GeoTiffException(
    ...
    /**
     * Looks up the Geodetic Datum as specified in the GeoTIFFWritingUtilities file.
     * The geotools
     * definition of the geodetic datum includes both an ellipsoid and a prime
     * meridian, but the
     * code in the GeoTIFFWritingUtilities file does NOT include the prime meridian,
     * as it is
     * specified separately. This code currently does not support user defined
     * datum.
     *
     ...
     *       transformation even if for
     *       the moment is not so clear to me how we could achieve that since if we
     *       have no clue about
     *       the coordinate transform what we are supposed to do in order to build a
     *       conversion, guess
     *       it? How could we pick up the parameters, should look for all and then
     *       guess the right
     *       transformation?
    ...
    /**
     * Set the projection parameters basing its decision on the projection name. I
     * found a complete
     ...
     * find all of
     * them. If you want go ahead and add support for the missing ones. I have
     * tested this code
     * against some geotiff files you can find on the geotiff website under the ftp
     * sample directory
     * but I can say that they are a real mess! I am respecting the specification
     * strictly while
     * many of those fields do not! I could make this method trickier and use
     * workarounds in order
     * to be less strict but I will not do this, since I believe it is may lead us
     * just on a very
     * dangerous path.
     *
    ...
    private ParameterValueGroup setParametersForProjection(
        ...
        try {
            ...
            if (name.equalsIgnoreCase("mercator_1SP")
                ...
                final double standard_parallel_1 = getGeoKeyAsDouble(ProjStdParallel1GeoKey, metadata);
    ...
    private static double getScaleFactor(final GeoTiffMetadata metadata) throws IOException {
        ...
        if (scale == null)
            scale = metadata.getGeoKey(ProjScaleAtNatOriginGeoKey);
        if (scale == null)
            return 1.0;
     ...
     * parameters name. I
     * saw that often people use the wrong geokey to store the false easting, we
     * cannot be too picky
    ...
    /**
     * Getting the false northing with a minimum of tolerance with respect to the
     * parameters name. I
     * saw that often people use the wrong geokey to store the false easting, we
     * cannot be too picky
     * we need to get going pretty smoothly.
    ...
    private static double getOriginLong(final GeoTiffMetadata metadata) throws IOException {
        ...
        if (origin == null)
            origin = metadata.getGeoKey(ProjNatOriginLongGeoKey);
        if (origin == null)
            origin = metadata.getGeoKey(ProjFalseOriginLongGeoKey);
        if (origin == null)
            origin = metadata.getGeoKey(ProjStraightVertPoleLongGeoKey);
        if (origin == null)
            origin = metadata.getGeoKey(ProjFalseNorthingGeoKey);
        if (origin == null)
            return 0.0;
     ...
     * parameters name. I saw
     * that often people use the wrong geokey to store the false easting, we cannot
     * be too picky we
    ...
    private static double getOriginLat(final GeoTiffMetadata metadata) throws IOException {
        ...
        if (origin == null)
            origin = metadata.getGeoKey(ProjNatOriginLatGeoKey);
        if (origin == null)
            origin = metadata.getGeoKey(ProjFalseOriginLatGeoKey);
        if (origin == null)
            return 0.0;
     ...
     * ProjLinearUnitsGeoKey</code> and the <code>ProjLinearUnitSizeGeoKey</code>.
     * The unit may
     * either be specified as a standard EPSG recognized unit, or may be user
     * defined.
     *
     * @return <code>Unit</code> object representative of the tags in the file.
     * @throws IOException if the<code>ProjLinearUnitsGeoKey</code> is not specified
     *                     or if unit is
     *                     user defined and <code>ProjLinearUnitSizeGeoKey</code> is
     *                     either not defined or does not
     *                     contain a number.
     */
     ...
     * which the raster space
     * will be transformed:
     * unknown,
     * Geocentric (rarely used),
     * Geographic,
     * Projected Coordinate System, or
     * user defined. If the coordinate system is a PCS, then only the PCS code need
     * be specified.
     * If the coordinate system does not fit into one of the standard registered
     * PCS'S,
     * but it uses one of the standard projections and datums, then it should be
     * documented
     * as a PCS model with "userdefined" type, requiring the specification of
     * projection parameters, etc.
     */
    public static final int GTModelTypeGeoKey = 1024;

    /** Unknown model type */
    public static final int ModelTypeUnknown = 0;
