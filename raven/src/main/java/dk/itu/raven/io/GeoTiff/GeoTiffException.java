 ...
 * wrong with the
 * GeoTiff file which produced it. In this case, the exception is reporting an
 * unsupported
 * coordinate system description or raster to model transform, or some other
 * unrecognized
 * configuration of the GeoTIFFWritingUtilities tags. By doing so, it attempts
 * to record enough
...
public final class GeoTiffException extends IOException {
    ...
    /**
     * Constructs an instance of <code>GeoTiffException</code> with the specified
     * detail message.
     *
     * @param metadata The metadata from the GeoTIFFWritingUtilities image causing
     *                 the error.
