 ...
 * K, and (X,Y,Z) is a
 * vector in model space. In most cases the model space is only two-dimensional,
 * in which case both
 * K and Z should be set to zero; this third dimension is provided in
 ...
 * and orientation in
 * the model coordinate space M. This may be done by specifying the location of
 * three of the four
 * bounding corner points. However, tiepoints are only to be considered exact at
 * the points
 * specified; thus defining such a set of bounding tiepoints does not imply that
 * the model space
 ...
 * will often be an
 * exact, affine transformation, this relationship can be defined using one set
 * of tiepoints and the
 * "ModelPixelScaleTag", described below, which gives the vertical and
 * horizontal raster grid cell
 * size, specified in model units.
 *
 * <p>
 * If possible, the first tiepoint placed in this tag shall be the one
 * establishing the location
 * of the point (0,0) in raster space. However, if this is not possible (for
 * example, if (0,0) is
 * goes to a part of model space in which the projection is ill-defined), then
 * there is no
 ...
 * may be specified
 * on a mesh over the raster image. However, the definition of associated grid
