 ...
 */
package dk.itu.raven.geotools;

/**
 * 6.3.2.1 Geographic CS Type Codes Note: A Geographic coordinate system
 * consists of both a datum
 * and a Prime Meridian. Some of the names are very similar, and differ only in
 * the Prime Meridian,
 * so be sure to use the correct one. The codes beginning with GCSE_xxx are
 * unspecified GCS which
 * use ellipsoid (xxx); it is recommended that only the codes beginning with
 * GCS_ be used if
 * possible. Ranges: 0 = undefined [ 1, 1000] = Obsolete EPSG/POSC Geographic
 * Codes [ 1001, 3999] =
 * Reserved by GeoTIFFWritingUtilities [ 4000, 4199] = EPSG GCS Based on
 * Ellipsoid only [ 4200,
 * 4999] = EPSG GCS Based on EPSG Datum [ 5000, 32766] = Reserved by
 * GeoTIFFWritingUtilities 32767 =
...
public final class GeoTiffGCSCodes {
	 ...
	 * 6.3.1.4 Codes
	 * Allows the definition of geocentric CS Linear units for user-defined GCS and
	 * for ellipsoids.
	...
	/**
	 * GeogAngularUnitSizeGeoKey Key ID = 2055 Type = DOUBLE Units: radians Allows
	 * the definition of
	 ...
	 * azimuths, in
	 * geographic coordinate systems. These may be used for defining azimuthal
	 * parameters for some
	 * projection algorithms, and may not necessarily be the same angular units used
	 * for lat-long.
	 */
	public static final int GeogAzimuthUnitsGeoKey = 2060;

	/**
	 * GeogCitationGeoKey Key ID = 2049 Type = ASCII Values = text General citation
	 * and reference
	 ...
	 * 6.3.2.3 Codes
	 * http://www.remotesensing.org/geotiff/spec/geotiff6.html#6.3.2.3 This key may
	 * be used to
	 * specify the coded ellipsoid used in the geodetic datum of the Geographic
	 * Coordinate System.
	...
	/**
	 * GeogGeodeticDatumGeoKey Key ID = 2050 Type = SHORT (code) Values = Section
	 * 6.3.2.2 Codes
	 * http://www.remotesensing.org/geotiff/spec/geotiff6.html#6.3.2.2 This key may
	 * be used to
	 * specify the horizontal datum, defining the size, position and orientation of
	 * the reference
	 * ellipsoid used in user-defined geographic coordinate systems. GeoKey
	 * Requirements for
	...
	/**
	 * GeogInvFlatteningGeoKey Key ID = 2059 Type = DOUBLE Units: none. Allows the
	 * specification of
	 * the inverse of user-defined Ellipsoid's flattening parameter (f). The
	 * eccentricity-squared
	 * e^2 of the ellipsoid is related to the non-inverted f by: e^2 = 2f - f^2
	 * Note: if the
	 ...
	 * Codes
	 * http://www.remotesensing.org/geotiff/spec/geotiff6.html#6.3.1.3 Allows the
	 * definition of
	...
	/**
	 * GeogLinearUnitSizeGeoKey Key ID = 2053 Type = DOUBLE Units: meters Allows the
	 * definition of
	 ...
	 * location of the Prime meridian for user-defined geographic coordinate
	 * systems. The default
	 ...
	 * GeogAngularUnits This key
	 * allows definition of user-defined Prime Meridians, the location of which is
	 * defined by its
	 ...
	 * 6.3.2.1 Codes
	 * http://www.remotesensing.org/geotiff/spec/geotiff6.html#6.3.2.1 This key may
	 * be used to
	 * specify the code for the geographic coordinate system used to map lat-long to
	 * a specific
	 * ellipsoid over the earth. GeoKey Requirements for User-Defined geographic CS:
	 * GeogCitationGeoKey GeogGeodeticDatumGeoKey GeogAngularUnitsGeoKey (if not
	 * degrees)
	 ...
	 * GeoKeys, this is
	 * provided to give an ASCII reference to published documentation on the overall
	 * configuration
