...
public class Visualizer {
    ...
    public void drawResult(IJoinResult results, ShapefileReader shapeFileReader,
        ...
        List<Entry<Object, Geometry>> features = vectorData.getFeatures();
        ...
        for (File file : files) {
            ...
            if (options.drawFeatures) {
                ...
                try {
                    ...
                    for (Entry<Object, Geometry> poly : features) {
    ...
    private void scaleEntries(List<Entry<Object, Geometry>> features) {
    ...
    private List<Polygon> getPolygons(List<Entry<Object, Geometry>> entries) {
    ...
    public BufferedImage drawRtree(RTree<Object, Geometry> rtree, List<Polygon> features, VisualizerOptions options)
            throws IOException {
        BufferedImage image = new BufferedImage(this.width, this.height, BufferedImage.TYPE_INT_RGB);
        Graphics2D vectorGraphics = image.createGraphics();
     ...
     */
    private void drawMbr(Node<Object, Geometry> node, Graphics2D graphics, Color color, int lineWidth) {
        graphics.setStroke(new BasicStroke(lineWidth));
        for (Node<Object, Geometry> child : TreeExtensions.getChildren(node)) {
            setColor(graphics, color);
